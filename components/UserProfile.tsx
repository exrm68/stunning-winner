...
const UserProfile: React.FC<UserProfileProps> = ({ onClose, botUsername }) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, 'settings', 'config'), s => {
      ...
      const rBot = (d.referralBotUsername || '').replace('@','').replace('/','').trim();
      const rApp = (d.referralAppName || '').replace('/','').replace('@','').trim();
      setCs(prev => ({
        ...
        // ✅ KEY FIX: empty string হলে আগের value রাখো — blank দিয়ে override করো না
        referralBotUsername:  rBot || prev.referralBotUsername,
        referralAppName:      rApp !== undefined ? rApp : prev.referralAppName,
      }));
